...
public class InfectionMover2 extends Operator {
    ...
    public double proposal() {
        ...
        int afterEligibleInfectionCount = 0;
        for (int i : blockCount.getValues()) {
            if (i == 0) {
                afterEligibleInfectionCount += 1;
            } else if (i > 0) {
                afterEligibleInfectionCount += 2;
            }
        }
        logHR += Math.log(afterEligibleInfectionCount) - Math.log(eligibleInfectionCount);
...
//		System.out.println(newick);
        ...
        if (debug) {
            ...
            Log.debug(String.format(
                    "proposal(): pre=%d, post=%d, eligibleBefore=%d, eligibleAfter=%d, logHR=%.5f, HR=%.5f",
                    pre, post, eligibleInfectionCount, afterEligibleInfectionCount, logHR, HR));
        }
